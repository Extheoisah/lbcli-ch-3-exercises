...
# Ensure the main wallet is available (may have been created in exercise 01.sh)
ensure_wallet_available "btrustwallet" "false"
check_cmd "Ensuring btrustwallet is available"

# Ensure the treasure wallet is available
ensure_wallet_available "treasurewallet" "false"
check_cmd "Ensuring treasurewallet is available"

# Generate an address for mining
...
# STUDENT TASK: Check wallet balance after receiving funds
...
P2SH_VALID=$(bitcoin-cli -regtest validateaddress $P2SH_ADDR | grep -o '"isvalid":[^,}]*' | grep -o '[^:]*$' | tr -d '" ')
...
# STUDENT TASK: Verify the message (CI-compatible version)
...
# For Bitcoin Core v28, we'll use a simplified approach for Taproot descriptors
echo "Creating a simplified Taproot descriptor for demonstration purposes..."

# Create a new taproot address that we know will work with descriptors
...
# Get the address info to extract the internal key
ADDR_INFO=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getaddressinfo "$NEW_TAPROOT_ADDR")
check_cmd "Getting address info"

# Extract the internal key (the x-only pubkey)
INTERNAL_KEY=$(echo "$ADDR_INFO" | grep -o '"desc": "[^"]*"' | cut -d'"' -f4 | grep -o "tr([^)]*)" | sed 's/tr(\(.*\))/\1/')
...
TAPROOT_DESCRIPTOR=$(bitcoin-cli -regtest -named getdescriptorinfo descriptor="$SIMPLE_DESCRIPTOR" | grep -o '"descriptor": "[^"]*"' | cut -d'"' -f4)
